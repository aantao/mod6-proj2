...
void requestBuy(int sockfd, Account &acc){
	int numbytes;
	char buf[MAXDATASIZE];
	char sym[5];  //stock symbol variable
	int quantity;
...
void requestLimitBuy(int sockfd, Account &acc){
	...
	printf("Max price per share? ");
	scanf("%f", &limit_price);
	// You: fill buffer with Limit Buy (b) request
	sprintf(buf, "b %s %d %f %f\n", sym, quantity, limit_price, acc.getBalance());
...
void requestSell(int sockfd, Account &acc){
	...
	// You: if so ..
		...
		//string name = strtok(buf, ",\n");
		string symbol = strtok(buf, ",\n");
		int q = atoi(strtok(NULL, ",\n"));  //Quantity 
		double cps = atof(strtok(NULL, ",\n"));  //Cost per Share 
		double proceeds = atof(strtok(NULL, ",\n"));  //Proceeds
		//You: record the sale in the client account
		if (proceeds > 0.0){
			acc.recordSale(symbol, q, cps);
		}
...
void requestLimitSell(int sockfd, Account &acc){
	...
	printf("Min price per share? ");
	scanf("%f", &limit_price);

	// You: if account has sufficient number of shares
	if(acc.hasNumShares(sym, quantity)){
		// You: prepare the buffer to send to server(s, symbol, quantity, limit_price)
		sprintf(buf, "s %s %d %f\n", sym, quantity, limit_price);
		// You: send sell request to server(s,symbol,quantity, limit_price);
		if (send(sockfd, buf, strlen(buf), 0) == -1)
			perror("send sell request");
		// You: get response from server
		if ((numbytes=recv(sockfd, buf, MAXDATASIZE-1, 0)) == -1) {
...
int main(int argc, char *argv[])
{
	...
	// should be: ./stock_client host account_name 
	if(argc != 3){
		fprintf(stderr,"usage: stock_client host account_name\n");
		exit(1);
	}
